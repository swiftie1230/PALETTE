...
def main():
    ...
    aa("--model_name", default="meta-llama/Meta-Llama-3.1-8B-Instruct", 
       choices=["gpt2-xl", "EleutherAI/gpt-j-6B", "meta-llama/Meta-Llama-3.2-3B-Instruct", "Qwen/Qwen2.5-1.5B-Instruct", "Qwen/Qwen2.5-3B-Instruct", "Qwen/Qwen2.5-7B-Instruct"])
    aa("--dataset", default="wikipedia", choices=["wikitext", "wikipedia"])
    aa("--layers", default=[9, 10, 11], type=lambda x: list(map(int, x.split(",")))) # 7
    aa("--to_collect", default=["mom2"], type=lambda x: x.split(","))
    aa("--sample_size", default=20, type=lambda x: None if x == "all" else int(x)) # 100000
    aa("--batch_tokens", default=None, type=lambda x: None if x == "any" else int(x))
    aa("--precision", default="float32", choices=["float64", "float32", "float16"]) # "float32"
    aa("--stats_dir", default=STATS_DIR)
    aa("--download", default=1, type=int, choices=[0, 1])
    args = parser.parse_args()
    
    if args.model_name == "meta-llama/Meta-Llama-3.2-3B-Instruct":
    ...
    elif args.model_name == "Qwen/Qwen2.5-1.5B-Instruct" or args.model_name == "Qwen/Qwen2.5-3B-Instruct" or args.model_name == "Qwen/Qwen2.5-7B-Instruct":
        tokenizer = AutoTokenizer.from_pretrained(args.model_name, trust_remote_code=True, token=token)
        model = AutoModelForCausalLM.from_pretrained(
            args.model_name, 
...
def layer_stats(
    ...
    def get_ds():
        ...
        elif model.config.architectures == ["Qwen2ForCausalLM"]:
            maxlen = model.config.model_max_length
        else:
    ...
    if model.config.architectures == ['LlamaForCausalLM']:
        ...
        npos = model.config.model_max_length
    else:
